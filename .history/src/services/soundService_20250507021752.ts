...
export const initBackgroundMusic = (): void => {
  ...
  // Try to start playing immediately (this will likely be blocked by browsers)
  // but we'll try anyway in case the user has already interacted with the page
  if (musicEnabled) {
    backgroundMusic.play().then(() => {
      console.log('Background music started playing automatically');
    }).catch(error => {
      console.log('Autoplay of background music was prevented by browser policy');
      // This is expected in most browsers without user interaction
    });
  }
