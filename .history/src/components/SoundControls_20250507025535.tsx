...
const SoundControls: React.FC = () => {
  ...
  // Initialize and check background music status on component mount
  useEffect(() => {
    // Initialize background music
    initBackgroundMusic();
    console.log('Background music initialized in SoundControls component');

    // Check if audio is already started (might have been started from LandingPage)
    if (isMusicEnabled()) {
      ...
      audio.play()
        .then(() => {
          console.log('Audio context started automatically in SoundControls');
          setAudioStarted(true);
        ...
        .catch(error => {
          console.log('Autoplay prevented by browser policy, user will need to click the Start Audio button');
          // We'll keep audioStarted as false so the Start Audio button will be shown
        });
    }
  }, []);
