  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Thank you for supporting SembezaAfrica. A confirmation has been sent to your email.",
      });
    }, 2000);
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="bg-game-background border border-game-primary/30 sm:max-w-[500px] max-h-[90vh] overflow-y-auto pb-16 sm:pb-6">
        <DialogHeader>
          <DialogTitle className="text-white text-2xl">Complete Your Donation</DialogTitle>
          <DialogDescription className="text-white/70">
            You are supporting SembezaAfrica with {cartItems.length} number{cartItems.length > 1 ? 's' : ''}.
          </DialogDescription>
        </DialogHeader>
